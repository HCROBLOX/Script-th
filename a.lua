-- ==============================================
-- MENU TAB ĐA NĂNG | SỬA TẬN GỐC LỖI KHÔNG HIỆN
-- Hiển thị ngay 100% | Mobile+PC | HC Roblox
-- ==============================================

-- 🔧 BƯỚC 1: KHAI BÁO DỊCH VỤ AN TOÀN
...
-- 🔧 BƯỚC 2: CHỜ PLAYERGUI CHẮC CHẮN
local PlayerGui = LocalPlayer:WaitForChild("PlayerGui", 15)
if not PlayerGui then
    warn("❌ LỖI: Không lấy được PlayerGui! Kiểm tra vị trí đặt script!")
    return
end
...
local function TaoUI()
    ...
    -- ✅ SỬA LỖI GỐC: Cấu hình ScreenGui đúng chuẩn
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "Menu_HCRoblox_Chinh"
    ScreenGui.Parent = PlayerGui
    ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ScreenGui.ResetOnSpawn = false -- QUAN TRỌNG: Không xóa khi đổi nhân vật
    ScreenGui.IgnoreGuiInset = true
    ScreenGui.Enabled = true -- BẬT HIỂN THỊ NGAY
    ScreenGui.DisplayOrder = 999 -- LUÔN HIỆN LÊN TRÊN CÙNG
...
local function CapNhatTatCa()
    ...
    NutLuonSang.CapNhat() NutTangHinh.CapNhat() NutKhongRoi.CapNhat() NutBatTu.CapNhat()
end
...
NutBayNhanh.MouseButton1Click:Connect(function() TrangThai.BayNhanh=not TrangThai.BayNhanh CapNhatTatCa() end)
...
NutLuonSang.MouseButton1Click:Connect(function()
    ...
    if TrangThai.LuonSang and RootPart then
        ...
        AnhSang.Brightness = 3.5 AnhSang.Range = 35 AnhSang.Color = Color3.new(1,1,0.7)
    else if AnhSang then AnhSang:Destroy() end end
    CapNhatTatCa()
end)
...
NutTangHinh.MouseButton1Click:Connect(function()
    ...
    if Character then for _,p in pairs(Character:GetChildren()) do if p:IsA("BasePart") or p:IsA("Decal") then p.Transparency=t end end end
    CapNhatTatCa()
end)
...
NutBatTu.MouseButton1Click:Connect(function()
    ...
    if Humanoid then Humanoid.MaxHealth=TrangThai.BatTu and math.huge or 100 Humanoid.Health=Humanoid.MaxHealth end
    CapNhatTatCa()
end)
...
UIS.InputBegan:Connect(function(i,gp)
    ...
    if i.KeyCode == Enum.KeyCode.F4 then GiaoDien.ChuyenMenu() end
end)
